    ...
    namespace Prism
    {
        ...
        public void SetInputBlack(byte v) { InputBlack = v; MarkLutDirty(); }
            ...
            // opacity doesn't affect the tonal LUT, no need to MarkLutDirty
        }
        ...
        /// </summary>
        public void SetContrast(float value) { Contrast = value; MarkLutDirty(); }
        ...
        private void EnsureCombinedLut()
        {
            ...
            if (levelsAreIdentity)
            {
                for (int i = 0; i < 256; i++)
                {
                    ...
                    float adjusted = ApplyTonalAdjustments(v / 255f);
                    _combinedLut[i] = (byte)(adjusted * 255f);
                }
            }
            else if (InputBlack < InputWhite && OutputBlack < OutputWhite)
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    float adjusted = ApplyTonalAdjustments(gamma);
                    _combinedLut[i] = (byte)Math.Clamp((int)(OutputBlack + adjusted * outRange), 0, 255);
                }
            }
            else
            {
        ...
        public float Lights = 0f;   // range -100..+100
        public float Shadows = 0f;  // range -100..+100

        public void SetLights(float value) { Lights = value; MarkLutDirty(); }
        public void SetShadows(float value) { Shadows = value; MarkLutDirty(); }

        private float ApplyTonalAdjustments(float normalized)
        {
            float c = Contrast / 100f;
            float contrasted = Math.Clamp((normalized - 0.5f) * (1f + c) + 0.5f, 0f, 1f);

            float lights = Lights / 100f;
            float shadows = Shadows / 100f;

            float lightsInfluence = contrasted * contrasted;
            float shadowsInfluence = (1f - contrasted) * (1f - contrasted);

            float adjusted = contrasted
                + lights * lightsInfluence * (1f - contrasted)
                + shadows * shadowsInfluence * contrasted;
